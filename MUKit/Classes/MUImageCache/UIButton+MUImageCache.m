...
// from FastImageCache
CGMutablePathRef _MUFICDCreateRoundedRectPath(CGRect rect, CGFloat cornerRadius)
{
...
@implementation UIButton (MUImageCache)
...
- (void)MUImageCache:(NSString *)urlString placeHolderImageName:(NSString *)imageName cornerRadius:(CGFloat)cornerRadius{
    ...
    [self.imageView sd_setImageWithURL:[NSURL URLWithString:urlString] completed:^(UIImage * _Nullable image, NSError * _Nullable error, SDImageCacheType cacheType, NSURL * _Nullable imageURL) {
        ...
        UIImage *tempImage = [self clipImage:image cornerRadius:cornerRadius];
        [self setBackgroundImage:tempImage forState:UIControlStateNormal];
...
- (UIImage *)clipImage:(UIImage *)originalImage cornerRadius:(CGFloat)cornerRadius{
    ...
    if (cornerRadius > 0) {
        ...
        CGFloat contentsScale = [UIButton contentsScale];
        ...
        if (cornerRadius > 0) {
            CGPathRef path = _MUFICDCreateRoundedRectPath(CGRectMake(0, 0,  imageRect.size.width,  imageRect.size.height), cornerRadius);
