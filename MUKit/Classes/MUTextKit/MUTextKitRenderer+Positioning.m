//
//  MUTextKitRenderer+Positioning.m
//  MUAsyncDisplayLayer
//
...
static const CGFloat MUTextKitRendererGlyphTouchHitSlop = 5.0;
static const CGFloat MUTextKitRendererTextCapHeightPadding = 1.3;

@implementation MUTextKitRenderer (Tracking)

- (NSArray *)rectsForTextRange:(NSRange)textRange measureOption:(MUTextKitRendererMeasureOption)measureOption
{
...
/**
 ...
 prevent a deadlock
 */
- (NSArray *)unlockedRectsForTextRange:(NSRange)textRange measureOptions:(MUTextKitRendererMeasureOption)measureOption layoutManager:(NSLayoutManager *)layoutManager textStorage:(NSTextStorage *)textStorage textContainer:(NSTextContainer *)textContainer
{
    ...
    [layoutManager enumerateLineFragmentsForGlyphRange:totalGlyphRange usingBlock:^(CGRect rect,
        ...
        if (!CGRectIsNull(lineRect)) {
            if (measureOption == MUTextKitRendererMeasureOptionBlock) {
...
- (CGRect)_internalRectForGlyphAtIndex:(NSUInteger)glyphIndex
                         measureOption:(MUTextKitRendererMeasureOption)measureOption
                           ...
                           textStorage:(NSTextStorage *)textStorage
{
    ...
    if (measureOption == MUTextKitRendererMeasureOptionCapHeight
        || measureOption == MUTextKitRendererMeasureOptionBlock) {
        ...
        // the text.
        CGFloat topPadding = MUTextKitRendererTextCapHeightPadding * descent;
...
- (void)enumerateTextIndexesAtPosition:(CGPoint)externalPosition usingBlock:(mu_text_component_index_block_t)block
{
    ...
    MUTextKitContext *lockingContext = self.context;
        ...
        for (NSInteger j = startIndex; j <= endIndex && !stop; j++) {
            ...
            [lockingContext performBlockWithLockedTextKitComponents:^(NSLayoutManager *layoutManager, NSTextStorage *textStorage, NSTextContainer *textContainer) {
                ...
                glyphRect = [self _internalRectForGlyphAtIndex:glyphIndex
                                                 measureOption:MUTextKitRendererMeasureOptionLineHeight
                                                 layoutManager:layoutManager
                                                 textContainer:textContainer
                                                   textStorage:textStorage];
            }];
            ...
            // Silly TextKit...
            if (!isValidGlyph || !CGRectContainsPoint(CGRectInset(glyphRect, -MUTextKitRendererGlyphTouchHitSlop, -MUTextKitRendererGlyphTouchHitSlop), currentPoint)) {
                continue;
            }
...
- (CGRect)trailingRect
{
    ...
    [self.context performBlockWithLockedTextKitComponents:^(NSLayoutManager *layoutManager, NSTextStorage *textStorage, NSTextContainer *textContainer) {
        ...
        NSArray *finalRects = [self unlockedRectsForTextRange:textRange measureOptions:MUTextKitRendererMeasureOptionLineHeight layoutManager:layoutManager textStorage:textStorage textContainer:textContainer];
