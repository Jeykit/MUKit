...
@interface UIButton (__MUImageCache) <MUImageRendererDelegate>
...
{
    if (image == nil && self.currentImage == nil) {
        return;
    }
...
- (void)MUImageRenderer:(MUImageRenderer *)render didDownloadImageURL:(NSURL *)url progressive:(UIImage *)progressiveImage{
    ...
    if (progressiveImage) {
        ...
        dispatch_main_async_safe(^{
            __strong typeof(weakSelf)self = weakSelf;
            [self setBackgroundImage:progressiveImage forState:UIControlStateNormal];
            [self setNeedsDisplay];
        });
    }
}
